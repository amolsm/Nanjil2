            ...
            if (!Comman.Comman.IsDataSetEmpty(DS))
            {
                ...
                if ((DS.Tables[1].Rows.Count != 0) && (DS.Tables[0].Rows.Count != 0))
                {
                    ...
                    foreach (DataRow row2 in DS.Tables[1].Rows)
                    {
                        ...
                        {
                            if (row2["OrderId"].ToString() == row["OrderId"].ToString() && row["Quantity"].ToString() != "0")
                            {
                                count = count + 1;
                ...
                if ((DS.Tables[3].Rows.Count != 0) && (DS.Tables[2].Rows.Count != 0))
                {
                    foreach (DataRow row3 in DS.Tables[3].Rows)
                    {
                        ...
                        {
                            if (row3["OrderId"].ToString() == row4["OrderId"].ToString() && row4["Quantity"].ToString()!="0")
                            {
                                count = count + 1;
                                sb.Append("<tr style='page-break-inside:avoid;'>");
                                sb.Append("<td class='tg-yw4l'   style='text-align:left'>");
                        ...
                        DS1 = billdata.GetSchemeAmountForBillwiseSalesSummary(row3["OrderId"],1);
                        if (!Comman.Comman.IsDataSetEmpty(DS1))
                        {
                            sb.Append("<tr style='page-break-inside:avoid;'>");
                            sb.Append("<td class='tg-yw4l'   style='text-align:left'>");
