...
{
    public partial class AgentwiseTraysReport : System.Web.UI.Page
    {
        MAgentwiseTray mat=new MAgentwiseTray();
        BAgentwiseTray bat = new BAgentwiseTray();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                DS = BindCommanData.BindCommanDropDwon("AgentID ", "AgentCode +' '+AgentName as Name  ", "AgentMaster", "IsActive=1");
                if (!Comman.Comman.IsDataSetEmpty(DS))
                {
                    dpAgent.DataSource = DS;
                    dpAgent.DataBind();
                    dpAgent.Items.Insert(0, new ListItem("--All Agent --", "0"));
                }
        ...
        protected void btngenrateBill_Click(object sender, EventArgs e)
        {
            ...
            int flag = 1;
            DS = bat.GetAgentwiseReturnTraysReport((Convert.ToDateTime(txtStartDate.Text)).ToString("dd-MM-yyyy"), (Convert.ToDateTime(txtEndDate.Text)).ToString("dd-MM-yyyy"), Convert.ToInt32(dpAgent.SelectedItem.Value), flag);
            if (!Comman.Comman.IsDataSetEmpty(DS))
            {
                ...
                sb.Append("<u>AgentWise Return Trays Report</u> <br/>");
                sb.Append("</th>");
                ...
                {
                    count1++;
                    sb.Append("<tr style='border-bottom:1px solid' style='page-break-inside:avoid;> <td colspan = '5'> &nbsp; </td> </tr>");
                    sb.Append("<tr style='border-bottom:1px solid'>");


                    sb.Append("<td class='tg-yw4l' colspan='5'  style='text-align:left'>");
                    sb.Append("<b>" + row["AgentCode"] + "</b>");
                    sb.Append("&nbsp;&nbsp;");
                    sb.Append("<b>" + row["AgentName"] + "</b>");
                    sb.Append("</td>");
                    int count = 0;
                    int outtrays = 0;
                    int intrays = 0;
                    ...
                    {
                        if (row["AgentCode"].ToString() == rows["AgentCode"].ToString())
                        {
                            count++;
                            sb.Append("<tr>");
                            ...
                            sb.Append("<td class='tg-yw4l'   style='text-align:left'>");
                            sb.Append(rows["D_Date"].ToString());
                            sb.Append("</td>");
                            ...
                            try
                            {
                                outtrays = Convert.ToInt32(rows["Trays_Dispached"]);
                            }
                            catch { outtrays = 0; }
                            ...
                            try
                            {
                                intrays = Convert.ToInt32(rows["Trays_Return"]);
                            }
                            catch { intrays = 0; }
                    ...
                    try
                    {
                        outtrays1 = Convert.ToInt32(row["Trays_Dispached"]);
                    }
                    catch { outtrays1 = 0; }
                    ...
                    try
                    {
                        intrays1 = Convert.ToInt32(row["Trays_Return"]);
                    }
                    catch { intrays1 = 0; }
